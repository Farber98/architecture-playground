...
type StateManager interface {
	TransitionToRunning(ctx *ExecutionContext) error
	TransitionToFailed(ctx *ExecutionContext) error
	TransitionToCompleted(ctx *ExecutionContext) error
}
...
func (sm *DefaultStateManager) TransitionToRunning(ctx *ExecutionContext) error {
	if err := ctx.Task.SetStatus(tasks.StatusRunning); err != nil {
		return err
	}

	// Attempt persistence for durability, but don't block execution on failure
	if err := sm.store.Update(ctx.Task.ID, ctx.Task.Status, ctx.Task.Result); err != nil {
		sm.logger.Task("failed to update running status", ctx.Task.ID, map[string]any{
			"error": err.Error(),
		})
...
func (sm *DefaultStateManager) TransitionToFailed(ctx *ExecutionContext) error {
	if setErr := ctx.Task.SetStatus(tasks.StatusFailed); setErr != nil {
	...
	if updateErr := sm.store.Update(ctx.Task.ID, ctx.Task.Status, ctx.Task.Result); updateErr != nil {
		sm.logger.Task("failed to update task failure state", ctx.Task.ID, map[string]any{
			"update_error":   updateErr.Error(),
			"original_error": ctx.Error.Error(),
		})
	}
...
func (sm *DefaultStateManager) TransitionToCompleted(ctx *ExecutionContext) error {
	if err := ctx.Task.SetStatus(tasks.StatusDone); err != nil {
		return err
	}

	// Attempt final persistence but don't fail successful execution for storage issues
	if err := sm.store.Update(ctx.Task.ID, ctx.Task.Status, ctx.Task.Result); err != nil {
		sm.logger.Task("failed to update final task state", ctx.Task.ID, map[string]any{
			"error":        err.Error(),
			"final_status": ctx.Task.Status.String(),
		})
